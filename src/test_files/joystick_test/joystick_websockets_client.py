...
async def main():
    ...
    while True:
        ...
        button_values = [joystick.get_button(
            i) for i in range(joystick.get_numbuttons())]
        axes_coordinates = [joystick.get_axis(
            i) for i in range(joystick.get_numaxes())]
        dpad_coordinates = joystick.get_hat(0)
        joystick_data = {"button_values": button_values,
                         "axes_coordinates": axes_coordinates, "dpad_coordinates": dpad_coordinates}
        async with websockets.connect(uri) as websocket:
            # converts dictionary into json file and sends it to server
